...
local function drawNotification(slot, notification, x, y, width, height, settings, drawList)
    ...
    if isMinified then
        ...
        if notificationData.lastSubtitleColors[notification.id] ~= fadedSubtitleColor then
            subtitleFont:set_font_color(fadedSubtitleColor);
            subtitleFont:set_outline_color(fadedSubtitleOutline);
            notificationData.lastSubtitleColors[notification.id] = fadedSubtitleColor;
        end
        -- Truncate if needed
    ...
    elseif isMinifying then
        ...
        -- Only set colors if changed (expensive D3D calls)
        if notificationData.lastTitleColors[notification.id] ~= minifyTitleColor then
            titleFont:set_font_color(minifyTitleColor);
            titleFont:set_outline_color(minifyTitleOutline);
            notificationData.lastTitleColors[notification.id] = minifyTitleColor;
        end
        ...
        -- Only set colors if changed (expensive D3D calls)
        if notificationData.lastSubtitleColors[notification.id] ~= baseSubtitleColor then
            subtitleFont:set_font_color(baseSubtitleColor);
            subtitleFont:set_outline_color(baseSubtitleOutline);
            notificationData.lastSubtitleColors[notification.id] = baseSubtitleColor;
        end
        ...
        titleFont:set_position_y(textY);
        -- Only set colors if changed (expensive D3D calls)
        if notificationData.lastTitleColors[notification.id] ~= fadedTitleColor then
            titleFont:set_font_color(fadedTitleColor);
            titleFont:set_outline_color(fadedTitleOutline);
            notificationData.lastTitleColors[notification.id] = fadedTitleColor;
        end
